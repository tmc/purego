...
// Syscall<N>Float1 tests and benchmarks.
...
// TestSyscallFloat1 verifies that the Syscall<N>Float1 family places the
...
func TestSyscallFloat1(t *testing.T) {
	...
	{
		got, _, _ := purego.Syscall1Float1(fn1, 10, f1)
		if want := uintptr(10 + floatPart); got != want {
			t.Errorf("Syscall1Float1: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall2Float1(fn2, 10, 20, f1)
		if want := uintptr(30 + floatPart); got != want {
			t.Errorf("Syscall2Float1: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall3Float1(fn3, 10, 20, 30, f1)
		if want := uintptr(60 + floatPart); got != want {
			t.Errorf("Syscall3Float1: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall4Float1(fn4, 10, 20, 30, 40, f1)
		if want := uintptr(100 + floatPart); got != want {
			t.Errorf("Syscall4Float1: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall8Float1(fn8, 1, 2, 3, 4, 5, 6, 7, 8, f1)
		if want := uintptr(36 + floatPart); got != want {
			t.Errorf("Syscall8Float1: got %d, want %d", got, want)
		}
	}
}
...
func TestSyscallFloat1_DistinctValues(t *testing.T) {
	...
	got, _, _ := purego.Syscall8Float1(fn8, 100, 200, 300, 400, 500, 600, 700, 800, 3.141)
	if want := uintptr(3600 + 3141); got != want {
		t.Errorf("Syscall8Float1: got %d, want %d", got, want)
	}
}

func BenchmarkSyscall4Float1(b *testing.B) {
	...
	var r uintptr
	for i := 0; i < b.N; i++ {
		r, _, _ = purego.Syscall4Float1(fn, 10, 20, 30, 40, 1.5)
	}
	b.StopTimer()
	if want := uintptr(100 + 1500); r != want {
		b.Fatalf("Syscall4Float1: got %d, want %d", r, want)
	}
}

func BenchmarkSyscall8Float1(b *testing.B) {
	lib := openABITestLib(b)
	...
	var r uintptr
	for i := 0; i < b.N; i++ {
		r, _, _ = purego.Syscall8Float1(fn, 1, 2, 3, 4, 5, 6, 7, 8, 1.5)
	}
	b.StopTimer()
	if want := uintptr(36 + 1500); r != want {
		b.Fatalf("Syscall8Float1: got %d, want %d", r, want)
	}
}
