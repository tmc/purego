...
// TestSyscallFloat1_32 verifies that the Syscall<N>Float1_32 family delivers
...
func TestSyscallFloat1_32(t *testing.T) {
	...
	{
		got, _, _ := purego.Syscall1Float1_32(fn1, 10, f1)
		if want := uintptr(10 + floatPart); got != want {
			t.Errorf("Syscall1Float1_32: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall2Float1_32(fn2, 10, 20, f1)
		if want := uintptr(30 + floatPart); got != want {
			t.Errorf("Syscall2Float1_32: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall3Float1_32(fn3, 10, 20, 30, f1)
		if want := uintptr(60 + floatPart); got != want {
			t.Errorf("Syscall3Float1_32: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall4Float1_32(fn4, 10, 20, 30, 40, f1)
		if want := uintptr(100 + floatPart); got != want {
			t.Errorf("Syscall4Float1_32: got %d, want %d", got, want)
		}
	}
	{
		got, _, _ := purego.Syscall8Float1_32(fn8, 1, 2, 3, 4, 5, 6, 7, 8, f1)
		if want := uintptr(36 + floatPart); got != want {
			t.Errorf("Syscall8Float1_32: got %d, want %d", got, want)
		}
	}
}
...
// float), which the float64 Syscall<N>Float1 family silently mis-delivers.
func TestSyscallFloat1_32_NotInterchangeableWithFloat64(t *testing.T) {
	...
	if got, _, _ := purego.Syscall4Float1_32(fnFloat, 10, 20, 30, 40, eps); got != wantCorrect {
	...
	if got, _, _ := purego.Syscall4Float1(fnFloat, 10, 20, 30, 40, float64(eps)); got == wantCorrect {
		t.Errorf("float64 family unexpectedly matched on a float param (got %d); "+
			"the single/double encodings should differ", got)
	}
}

func BenchmarkSyscall4Float1_32(b *testing.B) {
	...
	var r uintptr
	for i := 0; i < b.N; i++ {
		r, _, _ = purego.Syscall4Float1_32(fn, 10, 20, 30, 40, 1.5)
	}
	b.StopTimer()
	if want := uintptr(100 + 1500); r != want {
		b.Fatalf("Syscall4Float1_32: got %d, want %d", r, want)
	}
}

func BenchmarkSyscall8Float1_32(b *testing.B) {
	lib := openABITestLib(b)
	...
	var r uintptr
	for i := 0; i < b.N; i++ {
		r, _, _ = purego.Syscall8Float1_32(fn, 1, 2, 3, 4, 5, 6, 7, 8, 1.5)
	}
	b.StopTimer()
	if want := uintptr(36 + 1500); r != want {
		b.Fatalf("Syscall8Float1_32: got %d, want %d", r, want)
	}
}
