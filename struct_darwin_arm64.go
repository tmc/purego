...
func copyStruct8ByteChunks(ptr unsafe.Pointer, size uintptr, addChunk func(uintptr)) {
	for offset := uintptr(0); offset < size; offset += 8 {
		...
		if remaining >= 8 {
			...
			// For the last partial chunk, mask off the remaining bytes
...
func estimateStackBytes(ty reflect.Type) int {
	...
	for i := 0; i < ty.NumIn(); i++ {
		...
		if arg.Kind() == reflect.Struct {
			...
			if hfa {
				...
				// Non-HFA/HVA small structs use int registers for byte-packing
				slotsNeeded := int((uintptr(size) + align8ByteMask) / align8ByteSize)
	...
	// Round total to 8-byte boundary using the mask
	if stackBytes > 0 {
		stackBytes = int((uintptr(stackBytes) + align8ByteMask) &^ align8ByteMask)
	}
	return stackBytes
}
