...
func BenchmarkInterleavedFloat32(b *testing.B) {
	...
	b.Cleanup(func() {
		if err := load.CloseLibrary(libHandle); err != nil {
			b.Fatalf("failed to close library: %v", err)
		}
	})

	b.Run("5args_float_at_3", func(b *testing.B) {
		b.ReportAllocs()
		sym, err := load.OpenSymbol(libHandle, "rmsnorm_shape_c")
		if err != nil {
			b.Fatalf("failed to open symbol: %v", err)
		}
	...
	b.Run("9args_float_at_4", func(b *testing.B) {
		b.ReportAllocs()
		sym, err := load.OpenSymbol(libHandle, "sdpa_shape_c")
		if err != nil {
			b.Fatalf("failed to open symbol: %v", err)
		}
